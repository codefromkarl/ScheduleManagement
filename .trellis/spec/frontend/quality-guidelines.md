...
For Dashboard content changes, run a dense-state browser case rather than only an empty state: include scheduled tasks on multiple dates plus completed, blocked, unplanned, and recent-change data; confirm desktop week mode shows concurrent Monday-to-Sunday placement, the 390px day calendar begins by 450px, and no next-task/completed-scheduled/risk/history detail blocks are duplicated in default page flow.
Native drag acceptance must include at least one real pointer `dragTo` path. Synthetic `DragEvent` remains useful for exact coordinate/conflict branches but cannot prove that overlays, stacking contexts, or disappearing sources are physically reachable.
