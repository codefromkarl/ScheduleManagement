...
- [x] Replace QQ direct `insertTask`/reschedule mutations with a pure proposal service and durable expiring `pending_confirmation` payload; confirm against the latest schedule before atomic apply.
- [x] For QQ `no_slot`, expose an explicit `save_unplanned` action that creates only after confirmation; retain change-constraints/optimize/cancel alternatives and create nothing on expiry/cancel.
...
- [x] Add structured proposal presenters and an inline-keyboard interaction handler with owner authorization, durable interaction claims, fast ACK, opaque proposal/action data, and text-command fallback using the same proposal state machine. Real client rendered no buttons despite API acceptance, so the capability is explicitly disabled pending platform permission.
- [x] Add optional cropped schedule-image rendering only for conflict/multi-move/cross-date proposals; preserve a complete text diff and prove image generation/upload failure falls back without mutation. Isolated render and real client image receipt passed.
...
- [x] Enforce one active schedule-proposal slot per owner with transactional supersession and explicit user copy; old button/text actions return stable no-op states and immediate status/note updates leave the slot unchanged.
- [x] Add versioned 15-minute expiry for base proposals: reads do not extend, expired confirmation only regenerates, and terminal cancelled/applied proposals cannot be revived. Change-time/duration versioning remains with their pending interaction work.
- [x] Add deterministic text-fallback `change_time` candidates: at most three same-day-first/no-move safe slots across a bounded seven-day search; exact selection creates a new un-applied preview and optimize-only moves never leak into normal candidates. Inline-button/other-natural-language selection remains pending platform capability.
- [ ] Add `change_duration` quick values plus natural-language other-duration parsing, 15-minute normalization, current selection state, new-version recomputation, and zero-write `no_slot` handling. Numeric text fallback for insert proposals is implemented; natural language, buttons, and atomic scheduled-task duration edits remain.
...
- [x] Implement deterministic image-trigger classification and bounded relevant-window rendering for moves/cross-date/occupied-no-slot proposals; unit-test simple exclusions and transport-failure fallback to the unchanged pending proposal. Batch multi-task rendering remains a future extension when batch QQ proposals exist.
...
- Passed (2026-08-21 dashboard performance and QQ proposal candidate): 70/70 Vitest, ESLint, TypeScript, Drizzle check, 30-route production build, Dashboard bundle budget, Compose parse, Trellis validation, and 7/7 isolated Playwright passed. Production-shaped browser measurement reduced initial planning reads from 15 API requests to 6, kept mobile CLS at zero, and moved low-frequency add/detail/settings code behind dynamic client boundaries. Migrations 0004/0005 were first validated in isolated databases; the later live proposal deployment applied both. The QQ proposal-image smoke rendered a 28,580-byte PNG locally.
- Passed and human-confirmed (2026-08-21 proposal-first QQ): created a mode-`600` pre-migration backup, applied migration 0005, and deployed the typed one-slot/15-minute proposal state machine. A real C2C preview produced exactly zero task/block/ChangeSet/reminder rows; cancel kept zero; old confirm returned a terminal no-op; explicit confirm produced exactly one task, one block, and one ChangeSet; repeated confirm stayed `1/1/1`; conflict-safe ChangeSet undo removed the test task/block. A real complex PNG was client-visible with zero business writes. Inline-keyboard HTTP was accepted but the client rendered no buttons and no interaction arrived, so `QQBOT_INLINE_KEYBOARD_ENABLED=false` now prevents invisible controls and preserves text fallback. App/worker are healthy with SQLite quick check `ok` and zero foreign-key violations.
- Expected but not passed: real cloud AI (the available `OPENAI_API_KEY` returned HTTP 401), live button rendering/interaction permission, and real mobile push delivery. PWA remains deferred.
- Follow-up gaps: cloud AI credentials, QQ inline-keyboard permission, optional Cloudflare Access/passwordless identity, optional future PWA re-enable, dedicated project health page, silent AI preference learning beyond the explicit suggestion, daily risk detail beyond the Dashboard link, and randomized property/idempotency tests remain open.
...
2. Send a temporary task with a free slot through QQ; Goalset stores only a pending preview, then explicit confirmation inserts it once and creates an auditable ChangeSet.
