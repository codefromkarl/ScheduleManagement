...
QQ schedule mutations use a proposal-first state machine: `received -> clarifying -> pending_confirmation -> applied | cancelled | expired`. A pure preview service computes placement/moves without calling the mutating `insertTask()` path. The pending command receipt stores an opaque proposal ID, canonical task intent, deterministic placement/move diff, and a base schedule fingerprint. Confirm or button interaction re-reads the current schedule and recomputes before one atomic apply; a changed result produces a new proposal rather than applying stale data.

For `no_slot`, the proposal offers `save_unplanned`, `change_constraints`, `optimize`, and `cancel`. The user confirmed that `save_unplanned` is allowed, but it remains an explicit mutation: only that action may create the task without a schedule block. Before confirmation there is no task row, schedule block, applied ChangeSet, or reminder outbox row.

The first QQ approval surface is a structured plain-text preview plus inline keyboard. Simple one-task proposals stay text-only; conflict, multi-move, or cross-date proposals additionally render a cropped schedule image while retaining the same textual diff and buttons. Button data carries only an opaque proposal ID plus action, never the task payload. The interaction handler authorizes the owner, durably claims the interaction ID, ACKs within the platform deadline, reloads the proposal, and then revalidates/applies. If inline keyboard delivery or capability fails, the same proposal falls back to `确认 <proposalId>`, `改时间 <proposalId>`, `改时长 <proposalId>`, and `取消 <proposalId>` text commands.

Image rendering is supplemental and fail-open to text, not to mutation. It shows only the relevant time window, uses a dashed block for the proposed task, before/after cues for moved work, and a locked visual for fixed schedules. An image failure must never invalidate the text proposal or silently apply it.

QQ mutation authorization is classified before execution. `create`, `reschedule`, `optimize/reorder`, and `save_unplanned` always produce a proposal. Explicit status transitions (`done`, `blocked`, and equivalent unambiguous task-state commands) and notes updates may apply immediately through the existing audited task-update service, then return an undo action tied to that ChangeSet. If an update also changes date/time, targets multiple possible tasks, or lacks an exact task identity, it becomes a proposal/clarification instead of an immediate mutation.

Undo is not a second scheduling implementation: the button/text action references the created ChangeSet, checks owner and expiry, and calls the existing conflict-aware undo boundary. If later changes make undo unsafe, Goalset explains the conflict and does not overwrite newer state.

Each QQ owner has one active schedule-proposal slot. Creating a new schedule-affecting proposal transactionally marks the previous pending proposal `superseded`, stores the replacement, and returns copy that names the replacement without exposing payload internals. Every button/text action resolves its explicit opaque proposal ID and checks that it is still the active slot; stale, superseded, cancelled, expired, and already-applied IDs return a stable no-op result. Immediate status/note updates neither supersede nor occupy this slot.

An active QQ schedule proposal expires 15 minutes after its latest complete preview version. `change_time` and `change_duration` create a new version and new expiry; message reads and repeated button delivery do not extend it. An expired confirm action replays the stored canonical intent against the latest snapshot into a replacement proposal, but never applies in the same interaction. Cancelled/applied proposals are terminal and cannot be refreshed.

`change_time` uses the deterministic scheduler to return at most three no-move safe slots plus `other_time`, `back`, and `cancel`. Candidate order prefers the same date, then the nearest date within the task deadline/availability constraints. Selecting a slot or parsing an `other_time` reply creates a new proposal version and full preview; it never applies immediately. Slots requiring moves are excluded and may appear only after an explicit optimize authorization.

`change_duration` renders 15/30/45/60/90/120-minute buttons, highlights the current normalized value, and provides `other_duration`, `back`, and `cancel`. Other-duration input is parsed then normalized to the 15-minute scheduling grain with the normalized value visible before confirmation. Every duration change recomputes placement into a new proposal/version/expiry. A resulting `no_slot` proposal exposes the same explicit save-unplanned/change/optimize/cancel state instead of creating anything.

Every QQ mutation success response may expose an undo action for exactly 15 minutes, keyed by its ChangeSet ID rather than task text. The handler checks owner, action deduplication, expiry, current ChangeSet state, and later-conflict safety before invoking the existing undo service. Repeated/expired/already-undone requests return stable no-op copy; a conflict returns explanation/recovery options and never overwrites newer state. After expiry, a user request becomes a new command/proposal instead of reviving the old undo action.

Schedule-image rendering is triggered only when `moves.length > 0`, a proposal crosses dates, at least two tasks are proposed/affected, a `no_slot` explanation materially depends on occupied nearby blocks, or an optimize proposal contains multiple before/after changes. The renderer derives a bounded relevant time window from the proposal/snapshot and never emits a full-day/week image when it adds no information. Simple safe placement, state/note updates, and save-unplanned-only actions remain text/keyboard. Rendering/upload failure is isolated transport failure: the same proposal ID stays pending and the complete text/keyboard fallback is sent.
